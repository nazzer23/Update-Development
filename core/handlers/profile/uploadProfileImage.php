...
if(isset($_FILES['image']) && (!isset($_POST['removePic']))){
    ...
    if($imageURL == "error") {
        ...
        $database->executeQuery("UPDATE users SET ProfilePictureID='{$lastPhotoID->PhotoID}' WHERE UserID='{$_SESSION['userID']}'");
        header('Location: '.strtok($_SERVER['HTTP_REFERER'], '?'));
    }
} else if(isset($_POST['removePic'])) {
    $database->executeQuery("UPDATE users SET ProfilePictureID=0 WHERE UserID='{$_SESSION['userID']}'");
    header('Location: '.strtok($_SERVER['HTTP_REFERER'], '?'));
} else {
    header('HTTP/1.0 500 Internal Server Error');
}
